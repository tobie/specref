...
var str = fs.readFileSync('./refs/biblio.json', 'utf8');
var json = JSON.parse(str);

var ietf = JSON.parse(fs.readFileSync('./refs/ietf.json', 'utf8'));
var PROPS =  [
    "id",
    "authors",
    "etAl",
...
function Bibref(raw) {
    this.raw = raw;
    this.all = cleanupRefs(addIds(createUppercaseRefs(expandRefs(raw))));
...
function addRefs(refs) {
    ...
    var _cleanedRefs = cleanupRefs(addIds(createUppercaseRefs(expandRefs(refs))));
    _merge(this.all, _cleanedRefs);
    _merge(this.reverseLookupTable, prepareReverseLookup(_cleanedRefs));
}

function _merge(target, source) {
    for (var k in source) {
        if (k in target) {
            throw new Error("Dupliate entry " + k);
        } else {
            target[k] = source[k];
        }
    }
...
function normalizeUrl(u) {
    ...
    // one where missing).
    u = url.parse(u);
    u = (u.hostname || "").replace(/^www\./, "") + (u.pathname || "").replace(/\/$/, "") + (u.search || "");
...
function get(ref, output) {
    ...
    if ((!this.all[ref]) && this.all[ref.toUpperCase()]) {
        output[ref] = { aliasOf: ref.toUpperCase() };
        ref = ref.toUpperCase();
    }
...
module.exports = new Bibref(json, ietf);
